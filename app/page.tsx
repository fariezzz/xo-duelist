...
const GameComponent = () => {
  const [turn, setTurn] = useState(0); // State giliran game kamu

  const handleTimeout = () => {
    console.log("Waktu habis! Pindah giliran otomatis.");
    setTurn(prev => prev + 1);
  };

  const { timeLeft, maxTime } = useTurnTimer(turn, handleTimeout);
  ...
  return (
    <div className="p-4 text-center">
      <h2 className="text-xl font-bold">Giliran ke: {turn + 1}</h2>
      ...
      <p className="mt-2 text-2xl font-mono">
        Sisa Waktu: <span className={timeLeft < 5 ? 'text-red-500 animate-pulse' : ''}>{timeLeft}s</span>
      </p>
